...
function ReviewPage() {
  ...
  // console.log('all review', reviewData)

  useEffect(() => {
    if (Array.isArray(reviewData)) {
      setLatestReview(reviewData.slice(0, 6));
    } else {
      setLatestReview([]); // fallback empty array
    }
  }, [reviewData]);
    ...
    '>
        {
          latestReview?.map((item, index) => (
            <ReviewCard key={index} rating={item.rating} image={item.user.photoUrl} text={item.comment} 
            name={item.user.name} role={item.user.description} />
          ))
        }
